...
if(filesize>1024*1024*2 && (suffix=="jpg" || suffix=="png" ||suffix=="jpeg" ||suffix=="gif" ||suffix=="bmp" ||suffix=="ico")){
        ...
        $.ajax({  
         url: "../upload/upload."+langcode+"?path="+b.replace("..","@@")+"&processid=AN"+processid+"&id="+a+"&a="+c,
         type: 'POST',  
         data: form,  
         async: false,
        cache: false,
...
function getCookie(a) {
    if (getCookiex(a) == null || getCookiex(a) == "" || getCookiex(a) == undefined) {
        if (getCookiex(a+"x") == null || getCookiex(a+"x") == "" || getCookiex(a+"x") == undefined) {
            $b=null;
            $.ajax({ 
                type: "get", 
                url: "data.php?action=cookie&name="+a, 
                cache:false, 
                async:false, 
                success: function(res){ 
                    setCookie(a+"x", res, "h12")
                    b=res;
                } 
            });
            return b;
        }else{
            return getCookiex(a+"x");
        }
    }else{
        return getCookiex(a);
    }
...
app = angular.module("app").config([ "$controllerProvider", "$compileProvider", "$filterProvider", "$provide","$httpProvider", function(a, b, c, d,$httpProvider) {
  ...
  $httpProvider.defaults.headers.get['Pragma'] = 'no-cache';
    ...
    b.otherwise("/app/index"), a.state("app", {
        ...
        url:"/recycle/:x",
        templateUrl:function() {
            return null !== getCookie("auth") && 1 == getCookie("auth").split("|")[9] ? "tpl."+langcode+"?page=91" :"tpl."+langcode+"?page=7";
        }
    }).state("app.mtype", {
        url:"/mtype/:x",
        templateUrl:function() {
            return null !== getCookie("auth") && 1 == getCookie("auth").split("|")[9] ? "tpl."+langcode+"?page=92" :"tpl."+langcode+"?page=7";
        }
    }).state("app.mtype_add", {
        url:"/mtype_add/:id",
        templateUrl:function() {
            return null !== getCookie("auth") && 1 == getCookie("auth").split("|")[9] ? "tpl."+langcode+"?page=93" :"tpl."+langcode+"?page=7";
        }
    }).state("app.brand_list", {
        url:"/brand_list/:x",
        templateUrl:function() {
            return null !== getCookie("auth") && 1 == getCookie("auth").split("|")[4] ? "tpl."+langcode+"?page=94" :"tpl."+langcode+"?page=7";
        }
    }).state("app.brand_add", {
        url:"/brand_add/:id",
        templateUrl:function() {
            return null !== getCookie("auth") && 1 == getCookie("auth").split("|")[4] ? "tpl."+langcode+"?page=95" :"tpl."+langcode+"?page=7";
        }
    }).state("app.zmt_api", {
        url:"/zmt_api",
        templateUrl:function() {
            return null !== getCookie("auth") && 1 == getCookie("auth").split("|")[1] ? "tpl."+langcode+"?page=96" :"tpl."+langcode+"?page=7";
        }
    }).state("app.zmt_article", {
        url:"/zmt_article",
        templateUrl:function() {
            return null !== getCookie("auth") && 1 == getCookie("auth").split("|")[1] ? "tpl."+langcode+"?page=97" :"tpl."+langcode+"?page=7";
        }
    }).state("app.sql", {
        url:"/sql",
    ...
    a.app = {
        ...
        if(getCookie("auth")!=null && getCookie("auth")!="" && getCookie("auth")!=undefined){
...
*/
    ...
    a.getauth = function() {
        ...
        a.choose=function(a,b){
            c.open({
                template:'<style>.box{margin:2px;padding:5px;display: inline-block;cursor:pointer;width: 50px;height: 50px;border-radius: 10px; vertical-align:top;border:solid 1px #EEEEEE;text-align: center;}.box:hover{background: #EEEEEE;}.thead td{padding:10px;font-size:15px;}</style><div class="modal-header"><span style="font-size:14px;">选择图片素材</span> <button class="btn btn-info btn-xs pull-right" ng-click="managepic()">管理素材</button></div><div><table width="100%"><thead><tr class="thead"><td></td><td>名称 <button class="btn btn-info btn-xs" ng-click="p1()">升序 ↑</button> <button class="btn btn-info btn-xs" ng-click="p2()">降序 ↓</button></td><td>大小 <button class="btn btn-info btn-xs" ng-click="p3()">升序 ↑</button> <button class="btn btn-info btn-xs" ng-click="p4()">降序 ↓</button></td><td>类型 <button class="btn btn-info btn-xs" ng-click="p5()">升序 ↑</button> <button class="btn btn-info btn-xs" ng-click="p6()">降序 ↓</button></td><td>修改时间 <button class="btn btn-info btn-xs" ng-click="p7()">升序 ↑</button> <button class="btn btn-info btn-xs" ng-click="p8()">降序 ↓</button></td><td>选择 </td></tr></thead><tbody><tr ng-repeat="x in file_list"><td><img alt="<img src={{x.filepic}} width=400>" ng-src="{{x.filepic}}" ng-click="choosepic(\''+a+'\',x.filename)" class="box"></td><td>{{x.filename}}</td><td>{{x.size/1000}}K</td><td>{{x.type}}</td><td>{{x.time|getLocalTime}}</td><td><button class="btn btn-info btn-xs" ng-click="choosepic(\''+a+'\',x.filename)">选择</button></td></tr></tbody></table><div></iframe>',
...
$scope.p1=function(){
    ...
    $http.get("data."+langcode+"?action=file&lang="+$scope.lang+"&p=p1").success(function(response) {
        toastr.clear();
        $scope.file_list = response.file_list;
    });
}
...
$scope.p2=function(){
    toastr.warning("数据加载中，请稍候..."); 
    $http.get("data."+langcode+"?action=file&lang="+$scope.lang+"&p=p2").success(function(response) {
        toastr.clear();
        $scope.file_list = response.file_list;
    });
}
...
$scope.p3=function(){
    toastr.warning("数据加载中，请稍候..."); 
    $http.get("data."+langcode+"?action=file&lang="+$scope.lang+"&p=p3").success(function(response) {
        toastr.clear();
        $scope.file_list = response.file_list;
    });
}
...
$scope.p4=function(){
    toastr.warning("数据加载中，请稍候..."); 
    $http.get("data."+langcode+"?action=file&lang="+$scope.lang+"&p=p4").success(function(response) {
        toastr.clear();
        $scope.file_list = response.file_list;
    });
}
...
$scope.p5=function(){
    toastr.warning("数据加载中，请稍候..."); 
    $http.get("data."+langcode+"?action=file&lang="+$scope.lang+"&p=p5").success(function(response) {
        toastr.clear();
        $scope.file_list = response.file_list;
    });
}
...
$scope.p6=function(){
    toastr.warning("数据加载中，请稍候..."); 
    $http.get("data."+langcode+"?action=file&lang="+$scope.lang+"&p=p6").success(function(response) {
        toastr.clear();
        $scope.file_list = response.file_list;
    });
}
...
$scope.p7=function(){
    toastr.warning("数据加载中，请稍候..."); 
    $http.get("data."+langcode+"?action=file&lang="+$scope.lang+"&p=p7").success(function(response) {
        toastr.clear();
        $scope.file_list = response.file_list;
    });
}
...
$scope.p8=function(){
    toastr.warning("数据加载中，请稍候..."); 
    $http.get("data."+langcode+"?action=file&lang="+$scope.lang+"&p=p8").success(function(response) {
        toastr.clear();
        $scope.file_list = response.file_list;
    });
}
...
app.filter('getLocalTime', function() { //可以注入依赖
