...
if($S_lv>0){
$S_fen=getrs("select * from ".TABLE."lv where L_id=".$S_lv,"L_fen");
$L_title=getrs("select * from ".TABLE."lv where L_id=".$S_lv,"L_title");
if($_SESSION["M_id"]==""){
box("该板块需要登录会员后浏览！",$C_dir."member","error");
}else{
if(getrs("select * from ".TABLE."member where M_id=".$_SESSION["M_id"],"M_fen")-$S_fen<0){
...
if(mysqli_num_rows($result) > 0) {
while($row = mysqli_fetch_assoc($result)) {
echo "<a href=\"item.php?id=".$row["B_id"]."\" class=\"list-group-item\" style=\"height:55px;\"><span style=\"color:#AAAAAA;\">[".$S_title."]</span> <span style=\"font-weight:bold\">".lang($row["B_title"])."</span><span style=\"float:right;font-size:12px;\">".getrs("select * from ".TABLE."member where M_id=".$row["B_mid"],"M_login")."<br>".$row["B_time"]."</span></a>";
}
}
?>
