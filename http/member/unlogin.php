...
if($_SESSION["M_login"]==""){
	...
	mysqli_query($conn,"insert into ".TABLE."member(M_login,M_pwd,M_email,M_fen,M_pic,M_regtime,M_from,M_need) values('临时用户".$genkey."','".strtoupper(md5($genkey))."','',0,'member.jpg','".date('Y-m-d H:i:s')."',0,'')");
	...
	$M_lv=getrs("select * from ".TABLE."member where M_id=".$_SESSION["M_id"],"M_lv");
	$L_discount=getrs("select * from ".TABLE."lv where L_id=".$M_lv,"L_discount");
